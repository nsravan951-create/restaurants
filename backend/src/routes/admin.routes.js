...
router.patch('/restaurants/:restaurantId/toggle', asyncHandler(async (req, res) => {
  ...
  await pool.query(
    'UPDATE restaurants SET is_active = CASE WHEN is_active = 1 THEN 0 ELSE 1 END WHERE id = ?',
    [restaurantId]
  );
...
router.get('/summary', asyncHandler(async (req, res) => {
  const [[ordersRow]] = await pool.query('SELECT COUNT(*) AS totalOrders FROM orders');
  const [[restaurantsRow]] = await pool.query('SELECT COUNT(*) AS totalRestaurants FROM restaurants');
  const [[activeAdsRow]] = await pool.query('SELECT COUNT(*) AS totalActiveAds FROM ads WHERE is_active = 1');
