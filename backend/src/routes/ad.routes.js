...
router.get('/active', asyncHandler(async (req, res) => {
  ...
  if (restaurantId) {
    query += ' AND (restaurant_id IS NULL OR restaurant_id = ?)';
...
router.post('/', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
  ...
  const [result] = await pool.query(
    `INSERT INTO ads (title, image_url, target_link, restaurant_id, is_active, starts_at, ends_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [parsed.title, parsed.imageUrl, parsed.targetLink, parsed.restaurantId, parsed.isActive ? 1 : 0, parsed.startsAt, parsed.endsAt]
  );

  return res.status(201).json({ message: 'Ad created', adId: result.insertId });
}));
...
router.put('/:adId', requireAuth(['super_admin']), asyncHandler(async (req, res) => {
  ...
  await pool.query(`UPDATE ads SET ${setClause} WHERE id = ?`, values);
