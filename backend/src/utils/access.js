...
async function ensureRestaurantAccess(user, restaurantId) {
  ...
  const [rows] = await pool.query('SELECT id, owner_user_id FROM restaurants WHERE id = ?', [restaurantId]);
  if (!rows.length) {
  ...
  if (user.role === 'kitchen' || user.role === 'staff') {
    const [userRows] = await pool.query('SELECT restaurant_id FROM users WHERE id = ?', [user.userId]);
