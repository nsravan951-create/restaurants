    ...
    await pool.query(`
      ...
      BEGIN
        ...
        ALTER TABLE orders ADD CONSTRAINT chk_orders_payment_method
          CHECK (payment_method IN ('online', 'cod', 'upi', 'cash', 'cashfree'));

        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'payment_provider'
        ) THEN
          ALTER TABLE orders ADD COLUMN payment_provider VARCHAR(30);
        END IF;

        UPDATE orders
        SET payment_provider = CASE
          WHEN payment_method IN ('cod', 'cash') THEN 'cash'
          WHEN payment_method = 'online' OR razorpay_order_id IS NOT NULL OR razorpay_payment_id IS NOT NULL THEN 'razorpay'
          WHEN payment_method = 'upi' OR upi_txn_ref IS NOT NULL THEN 'upi'
          ELSE 'unknown'
        END
        WHERE payment_provider IS NULL;

        ALTER TABLE orders ALTER COLUMN payment_provider SET DEFAULT 'cash';
        ALTER TABLE orders DROP CONSTRAINT IF EXISTS chk_orders_payment_provider;
        ALTER TABLE orders ADD CONSTRAINT chk_orders_payment_provider
          CHECK (payment_provider IN ('cash', 'cashfree', 'razorpay', 'upi', 'unknown'));
